...
//  private val blocking_client = JNSafGrpc.blockingStub(channel)
  ...
  private def startStream(fileUri: FileResourceUri): Option[String] = {
    ...
    if (!doneSignal.await(6, TimeUnit.MINUTES)) {
  ...
  def taintAnalysis(apkUri: FileResourceUri, approach:
    TaintAnalysisApproach.Value): Option[TaintResult] = {
    ...
    try {
      ...
      val responseFuture: Future[TaintAnalysisResponse] =
        client.taintAnalysis(request)
